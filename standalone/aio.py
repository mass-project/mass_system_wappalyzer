...
async def fetch(url, match_queue, result_queue, resolver, client_timeout):
    try:
        ...
        timeout = ClientTimeout(total=client_timeout, connect=None, sock_connect=None, sock_read=None)
...
async def run_requests(loop, url_queue, match_queue, result_queue, num_connections, nameserver, client_timeout):
    ...
    while True:
        ...
        job = await scheduler.spawn(fetch(url, match_queue, result_queue, resolver, client_timeout))
...
def aio_handle_requests(url_queue, match_queue, result_queue, num_connections, nameserver=None, client_timeout=5):
    setproctitle("wappalyzer: aio_handle_requests")

    loop = asyncio.get_event_loop()
    #loop = uvloop.new_event_loop()
    #asyncio.set_event_loop(loop)
    loop.run_until_complete(run_requests(loop, url_queue, match_queue, result_queue, num_connections, nameserver, client_timeout))
